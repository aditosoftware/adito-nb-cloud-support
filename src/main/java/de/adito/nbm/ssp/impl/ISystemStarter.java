...
interface ISystemStarter
{
   ...
   * @return true if the server reports it is starting the system, false otherwise
   ...
   */
  default boolean doStartSystem(@NonNull String pUsername, @NonNull DecodedJWT pJWT, @NonNull String pSystemId) throws UnirestException, AditoSSPException
  {
    ...
    HttpResponse<JsonNode> listSystemsResponse = Unirest.post(getSystemStartUrl())
        .headers(headers)
        .body(systemDetailsBody)
        .asJson();
    HttpUtil.verifyStatus(listSystemsResponse);
    // TODO check response body instead of returning true
    return true;
  }

  @NonNull
  String getSystemStartUrl();
